					...
					public void run(){
						...
						if(os.equalsIgnoreCase("linux")) {
							...
							if(environment_global.contains("browserstack"))
								writer.println("/usr/bin/apache-maven-3.2.5/bin/mvn verify -Dwebdriver.driver=" + brs.toLowerCase() + " -Dwebdriver.safari.install=false -Dwebdriver.base.url=" + url + " -Dcuke.placester.env=browserstack_parallel -Dunlock.setting.value=" + y + " -Dcucumber.options=\"--glue classpath:com/placester/web/steps src/main/features --tags " + tag + " --format json-pretty:target/cucumber-report-myReport.json --format html:target/cucumber-html-report-myReport\" >TestResults/TestReport_" + brs + ver + "_" + env + "_thread" + y);
							else if(environment_global.contains("local") && number_of_records_in_config_file <= 3)
								writer.println("/usr/bin/apache-maven-3.2.5/bin/mvn verify -Dwebdriver.driver=" + brs.toLowerCase() + " -Dwebdriver.safari.install=false -Dwebdriver.base.url=" + url + " -Dcuke.placester.env=local -Dunlock.setting.value=yes -Dcucumber.options=\"--glue classpath:com/placester/web/steps src/main/features --tags " + tag + " --format json-pretty:target/cucumber-report-myReport.json --format html:target/cucumber-html-report-myReport\" >TestResults/TestReport_" + brs + ver + "_" + env + "_thread" + y);
							else if(environment_global.contains("local") && number_of_records_in_config_file > 3)
								writer.println("/usr/bin/apache-maven-3.2.5/bin/mvn verify -Dwebdriver.driver=" + brs.toLowerCase() + " -Dwebdriver.safari.install=false -Dwebdriver.base.url=" + url + " -Dcuke.placester.env=local -Dunlock.setting.value=no -Dcucumber.options=\"--glue classpath:com/placester/web/steps src/main/features --tags " + tag + " --format json-pretty:target/cucumber-report-myReport.json --format html:target/cucumber-html-report-myReport\" >TestResults/TestReport_" + brs + ver + "_" + env + "_thread" + y);
							writer.close();
						}
						if(os.equalsIgnoreCase("windows") && environment_global.contains("browserstack"))
							cmd = "cmd /c start mvn verify test -Dwebdriver.driver=" + brs + " -Dwebdriver.safari.install=false -Dwebdriver.base.url=" + url + " -Dcuke.placester.env=browserstack_parallel -Dunlock.setting.value=" + y + " -Dcucumber.options=\"--glue classpath:com/placester/web/steps src/main/features --tags " + tag + " --format json-pretty:target/cucumber-report-myReport.json --format html:target/cucumber-html-report-myReport\"";
						else if(os.equalsIgnoreCase("windows") && environment_global.contains("local"))
							cmd = "cmd /c start mvn verify test -Dwebdriver.driver=" + brs + " -Dwebdriver.safari.install=false -Dwebdriver.base.url=" + url + " -Dcuke.placester.env=local -Dunlock.setting.value=" + y + " -Dcucumber.options=\"--glue classpath:com/placester/web/steps src/main/features --tags " + tag + " --format json-pretty:target/cucumber-report-myReport.json --format html:target/cucumber-html-report-myReport\"";
						else {
